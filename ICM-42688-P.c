...
uint8_t ICM42688_INIT(I2C_HandleTypeDef *I2Cx)
{
    ...
    HAL_I2C_Mem_Read(I2Cx, ICM42688_ADDRESS, WHO_AM_I, 1, &check, 1, i2c_timeout);
    ...
    if (check == 104)  // Verify if the address is 0x68
        {
            ...
            // 0x001 to Active High, 0x010 to set Push Pull, BIT 2 = 0 to set Latched mode
            Data = 0x3;
...
void MPU6050_Read_ACCEL(I2C_HandleTypeDef *I2Cx, ICM42688_t *DataStruct)
{
    ...
    //This ICM-42688 does not need to divide the value by a scale
}
